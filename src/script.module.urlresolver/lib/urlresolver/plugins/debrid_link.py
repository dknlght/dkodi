...
USER_AGENT = 'URLResolver for Kodi/{0}'.format(common.addon_version)
...
class DebridLinkResolver(UrlResolver):
    ...
    def __initiate_transfer(self, transfer_id, interval=5):
        try:
            ...
            if transfer_info:
                ...
                with common.kodi.ProgressDialog('URLResolver Debrid-Link Transfer', line1, line2, line3) as pd:
    ...
    def authorize_resolver(self):
        ...
        with common.kodi.CountdownDialog('URLResolver Debrid-Link Authorization', line1, line2,
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver/?mode=auth_dl)"/>'.format(cls.__name__, i18n('auth_my_account')))
