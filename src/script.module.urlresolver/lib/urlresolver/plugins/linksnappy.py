...
USER_AGENT = 'URLResolver {0} for Kodi {1}'.format(common.addon_version, kodi.kodi_version())
...
## Provides a zip-file of a torrent, not used in urlresolver
...
class LinksnappyResolver(UrlResolver):
    ...
    def __initiate_transfer(self, media_id, interval=5):
        ...
        if transfer_info:
            ...
            with common.kodi.ProgressDialog('URLResolver Linksnappy transfer', line1, line2, line3) as pd:
            ...
            if torrent:
                ...
                if stream['status'] != 'OK':
                    ...
                    if transfer_info.get('percent') != 100:
                        ...
                        with common.kodi.ProgressDialog('URLResolver Linksnappy transfer', line1, line2) as pd:
    ...
    def get_settings_xml(cls):
        ...
        xml.append(
            '<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver/?mode=auth_ls)" option="close"/>'.format(
                cls.__name__, i18n('auth_my_account')
            )
        )

        xml.append(
            '<setting id="{0}_reset" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver/?mode=reset_ls)"/>'.format(
