...
class UpToBoxResolver(UrlResolver):
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver/?mode=auth_ub)"/>'.format(cls.__name__, i18n('auth_my_account')))
        xml.append('<setting id="{0}_reset" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver/?mode=reset_ub)"/>'.format(cls.__name__, i18n('reset_my_auth')))
