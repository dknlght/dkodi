...
def test_video_exists( page_url ):
    logger.info("[vidzur.py] test_video_exists(page_url='%s')" % page_url)
...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("[vidzur.py] url="+page_url)
    ...
    video_urls = []
    video_urls.append( [ scrapertools.get_filename_from_url(media_url)[-4:]+" [vidzur]",media_url])
