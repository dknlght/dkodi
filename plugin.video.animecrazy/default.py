...
#animecrazy
ADDON=dc=xbmcaddon.Addon(id='plugin.video.animecrazy')
addonPath=os.getcwd()
...
PATH = "AnimeCrazy"  #<---- PLUGIN NAME MINUS THE "plugin.video"          
...
def HOME():
        ...
        addDir('Recently Updated','http://www.animecrazy.net',17,icon)
...
def LatestEpisodes(url):
        icon = os.path.join(xbmc.translatePath( addonPath ), 'icon.png')
        addDir('Last Two Days','http://www.animecrazy.net',18,icon)
        addDir('Week 1','http://www.animecrazy.net',18,icon)
        addDir('Week 2','http://www.animecrazy.net',18,icon)
        addDir('Week 3','http://www.animecrazy.net',18,icon)
...
def GetWeeks(url,strgroup):
        ...
        if("Week" in strgroup):
                ...
                for vcontent in variablematch:
                        ...
                        addVideoInfo_Image('http://www.animecrazy.net/' + vurl[0], vname[0])
                        #addDir(vname[0],url+vurl[0],9,vimg[0])
        else:
                group=re.compile('<h3> Latest Anime Releases </h3>((.|\s)*?)<div id="group2">').findall(link[0][0])[0][0]
